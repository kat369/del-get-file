...
app.post("/create", upload.single("pdf"), async (req, res, next) => {
  ...
  try {
    ...
    let pdffile = await db
      ...
      .updateOne(
        { _id: mongoid },
        { $set: { pdfdata: req.file, destination: path, fileurl: url } }
      );
  ...
  async (req, res, next) => {
    ...
    if (req.file === undefined) {
      try {
        ...
        var url = `uploads/${obj.volume}/${obj.issue}/${req.query.oldfile}`;
      ...
      try {
        ...
        var url = `uploads/${obj.volume}/${obj.issue}/${req.file.originalname}`;
